...
numero INT,     
fecha_agregado DATE,     
id_usuario INT,     FOREIGN KEY (id_usuario) REFERENCES Usuarios(id) ON DELETE CASCADE );  
...
id_usuario INT,     
id_curso INT,     
progreso FLOAT, -- Porcentaje del progreso en el curso     
fecha_actualizacion DATETIME DEFAULT CURRENT_TIMESTAMP,     
FOREIGN KEY (id_usuario) REFERENCES Usuarios(id) ON DELETE CASCADE,     
FOREIGN KEY (id_curso) REFERENCES Cursos(id) ON DELETE CASCADE );  -- Procedimientos almacenados (actualizados si es necesario)  
...
CREATE TABLE Modulos (
    ...
    archivo BLOB,  -- Puede almacenar el archivo en binario o la URL/archivo del archivo subido
    curso_id INT,    
    estado ENUM('Activo', 'Inactivo') DEFAULT 'Activo',
    fecha_creacion DATETIME DEFAULT CURRENT_TIMESTAMP,     
...
BEGIN
    INSERT INTO Cursos (nombre, descripcion, detalles, categoria_id, id_instructor, costo, estado, calificacion_promedio) 
    VALUES (nombre, descripcion, detalles, categoria_id, id_instructor, costo, estado, calificacion_promedio);
END //
DELIMITER ;

-- PROCEDURE: Agregar Módulo
DELIMITER //
...
-- PROCEDURE: Obtener Módulos por Curso
DELIMITER //
